...
int main(int argc, char **argv)
{
    ...
    cxxopts::Options options("metrics_collector", "System metrics collector for Azure Storage");
    
    options.add_options()
        ("c,config", "Configuration file path", cxxopts::value<std::string>()->default_value("/mnt/d/Development/CPP/backup-utility-cpp/app/config.json"))
    ...
    try {
        ...
        metrics::AzureUploader uploader(
            config.get_azure_connection_string(),
        ...
        while (running) {
            // Sleep for a while
            std::this_thread::sleep_for(std::chrono::seconds(config.get_collection_interval_seconds()));
            
            // Get metrics and upload them
            auto metrics_data = collector.get_metrics_and_clear();
            if (!metrics_data.empty()) {
                if (uploader.upload_metrics(metrics_data)) {
                    spdlog::info("Successfully uploaded {} metrics", metrics_data.size());
                } else {
