...
namespace metrics{
    ...
    void ConfigParser::parse_config(const nlohmann::json& config){
        ...
        if(config.contains("collection_interval_seconds")){
            collection_interval_seconds_ = config["collection_interval_seconds"].get<int>();
        }
        ...
        if(config.contains("azure")){
            ...
            if (azure.contains("storage_account_url")) {
                azure_connection_string_ = azure["storage_account_url"].get<std::string>();
            }
            if (azure.contains("storage_account_name")) {
                azure_container_name_ = azure["storage_account_name"].get<std::string>();
            }
            if (azure.contains("storage_account_key")) {
                azure_container_name_ = azure["storage_account_key"].get<std::string>();
            }
        }
        ...
        if (config.contains("metrics") && config["metrics"].is_array()) {
            for (const auto& metric : config["metrics"]) {
                ...
                if (metric.contains("parameters") && metric["parameters"].is_object()) {
    ...
    MetricType ConfigParser::string_to_metric_type(const std::string& type_str) {
        if (type_str == "cpu_usage") return MetricType::CPU_USAGE;
        if (type_str == "cpu_temperature") return MetricType::CPU_TEMPERATURE;
        if (type_str == "ram_usage") return MetricType::RAM_USAGE;
        if (type_str == "disk_usage") return MetricType::DISK_USAGE;
        if (type_str == "network_bandwidth") return MetricType::NETWORK_BANDWIDTH;
        if (type_str == "process_count") return MetricType::PROCESS_COUNT;
