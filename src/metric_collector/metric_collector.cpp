...
void MetricCollector::register_providers(
...
void MetricCollector::start()
{
    ...
    running_ = true;
    collection_thread_ =
        std::thread(&MetricCollector::collection_routine, this);
...
void MetricCollector::collection_routine()
{
    while (running_)
    {
        ...
        std::this_thread::sleep_for(
            std::chrono::seconds(collection_interval_seconds_));
    }
}
...
void MetricCollector::collect_metrics()
{
    ...
    for (auto &provider : providers_)
    {
        try
        {
            ...
            new_metrics.insert(new_metrics.end(),
                               metrics.begin(),
                               metrics.end());
        }
    ...
    metrics_buffer_.insert(metrics_buffer_.end(),
                           new_metrics.begin(),
                           new_metrics.end());
}
...
nlohmann::json MetricCollector::get_metrics_and_clear()
{
    ...
    for (const auto &metric : metrics_buffer_)
    {
        ...
        nlohmann::json metric_json = {{"name", metric.name},
                                      {"unit", metric.unit},
                                      {"value", metric.value},
