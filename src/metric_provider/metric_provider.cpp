...
std::vector<MetricData> RamUsageProvider::collect()
{
    ...
    double usage_percent =
        100.0 * static_cast<double>(used_mem) / static_cast<double>(total_mem);

    MetricData usage_data{"ram_usage_percent",
                          "%",
                          usage_percent,
                          std::chrono::system_clock::now()};

    MetricData total_data{"ram_total",
                          "KB",
                          static_cast<double>(total_mem),
                          std::chrono::system_clock::now()};

    MetricData used_data{"ram_used",
                         "KB",
...
std::vector<MetricData> DiskUsageProvider::collect()
{
    ...
    if (statvfs(mount_point.c_str(), &stat) == 0)
    {
        ...
        MetricData usage_data{"disk_usage_percent",
                              "%",
                              usage_percent,
                              std::chrono::system_clock::now()};

        MetricData total_data{"disk_total",
                              "bytes",
                              total,
                              std::chrono::system_clock::now()};

        MetricData used_data{"disk_used",
                             "bytes",
                             used,
                             std::chrono::system_clock::now()};
